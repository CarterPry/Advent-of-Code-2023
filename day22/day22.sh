...
function check_1d() {
  ...
  local x3=$3
  local x4=$4
  if [[ $x1 -ge $x3 ]] && [[ $x1 -le $x4 ]]; then
    return 0
  fi
  if [[ $x2 -ge $x3 ]] && [[ $x2 -le $x4 ]]; then
    return 0
  fi
  if [[ $x1 -lt $x3 ]] && [[ $x2 -gt $x4 ]]; then
    return 0
  fi
  return 1
}
function check_intersects() {
  ...
  while IFS= read -r line; do
    ...
    [[ "$id" == "" ]] && continue
    if check_1d $x1 $x2 $x3 $x4 && check_1d $y1 $y2 $y3 $y4; then
      echo "$id"
    fi
  done <<< "${bricks[$layer]}"
}
...
while read x1 y1 z1 x2 y2 z2; do
  ...
  ((id++))
done < <(cat $FILE | tr '~,' ' ' | sort -nk3)

UNSAFE=$(for i in "${!deps[@]}";do printf "%s=%s\n" "$i" "${deps[$i]///,}";done | grep -v ',' | cut -d'=' -f2 | sort -u | wc -l)
